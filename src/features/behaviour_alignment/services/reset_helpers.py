...
def clear_common_selections(app_context: Any) -> None:
    ...
    app_context.behaviour_table_panel.clear_table()
...
def clear_photometry_app_specific_selections(app_context: Any) -> None:
    ...
    app_context.behaviour_options_panel.create_behaviour_options(
        app_context.no_behaviours, destroy_frame=True
    )
    app_context.plot_service.handle_figure_display_selection(None)
