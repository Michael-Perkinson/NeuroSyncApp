...
class TelemetryStaticSettingsStore:
    ...
    def save_static_inputs(self, df) -> None:
        ...
        self._cluster_settings_path().write_text(
            json.dumps(data["clusters"]), encoding="utf-8"
        )
        self._stim_settings_path().write_text(
    ...
    def load_static_inputs(self) -> dict:
        ...
        def load_data(primary_path: Path, legacy_path: Path, data_key: str) -> None:
        ...
        load_data(
            self._cluster_settings_path(),
            self._legacy_cluster_settings_path(),
            "clusters",
        )
        load_data(
            self._stim_settings_path(),
            self._legacy_stim_settings_path(),
            "stimulations",
        )
    ...
    @staticmethod
    def _cluster_settings_path() -> Path:
        return config_file_path(
            "telemetry_alignment",
            "cluster_static_settings.json",
        )

    @staticmethod
    def _stim_settings_path() -> Path:
        return config_file_path(
            "telemetry_alignment",
            "stim_static_settings.json",
        )

    @staticmethod
    def _legacy_cluster_settings_path() -> Path:
        return legacy_repo_file_path("cluster_static_settings.json")

    @staticmethod
    def _legacy_stim_settings_path() -> Path:
        return legacy_repo_file_path("stim_static_settings.json")
