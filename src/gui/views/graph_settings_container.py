...
class GraphSettingsContainer(ttk.Frame):
    @staticmethod
    def _bind_settings_manager_var(tk_var, settings_manager, attribute_name: str) -> None:
        setattr(settings_manager, attribute_name, tk_var.get())
        tk_var.trace_add(
            "write",
            lambda *_: setattr(settings_manager, attribute_name, tk_var.get()),
        )
    ...
    def __init__(self, parent, widgets_to_include=None,
        ...
        self.duration_box_placement = bind_tk_var(
            tk.StringVar(), self.view_state, "duration_box_placement"
        )
        self.table_treeview = None
        self.activity_data_var = bind_tk_var(
            tk.BooleanVar(), self.view_state, "activity_data_enabled"
        )
        self.temperature_data_var = bind_tk_var(
            tk.BooleanVar(), self.view_state, "temperature_data_enabled"
        )
        ...
        self.default_column_names = {'Behaviours/events': '',
                                     'Start Time': '',
                                     'End Time': ''}
        self.display_duration_box_var = bind_tk_var(
            tk.BooleanVar(), self.view_state, "display_duration_box"
        )
        self.num_instances_box_var = bind_tk_var(
            tk.BooleanVar(), self.view_state, "num_instances_box"
        )
        self.display_duration_box_var.set(
            bool(self.settings_manager.display_duration_box_var)
        )
        self.display_duration_box_var.trace_add(
            "write",
            lambda *_: setattr(
                self.settings_manager,
                "display_duration_box_var",
                bool(self.display_duration_box_var.get()),
            ),
        )
        self.num_instances_box_var.set(bool(self.settings_manager.num_instances_box_var))
        self.num_instances_box_var.trace_add(
            "write",
            lambda *_: setattr(
                self.settings_manager,
                "num_instances_box_var",
                bool(self.num_instances_box_var.get()),
            ),
        )
        self.time_unit_var = bind_tk_var(
            tk.StringVar(), self.view_state, "time_unit"
        )
        self.time_input_unit_var = bind_tk_var(
            tk.StringVar(), self.view_state, "time_input_unit"
        )
        self.x_gridlines_var = bind_tk_var(
            tk.StringVar(), self.view_state, "x_gridlines"
        )
        self.y_gridlines_var = bind_tk_var(
            tk.StringVar(), self.view_state, "y_gridlines"
        )
        ...
        self.x_axis_min_var = bind_tk_var(
            tk.StringVar(), self.view_state, "x_axis_min"
        )
        self.x_axis_max_var = bind_tk_var(
            tk.StringVar(), self.view_state, "x_axis_max"
        )
        self.y_axis_min_var = bind_tk_var(
            tk.StringVar(), self.view_state, "y_axis_min"
        )
        self.y_axis_max_var = bind_tk_var(
            tk.StringVar(), self.view_state, "y_axis_max"
        )
        self.selected_behaviour_to_zero = bind_tk_var(
            tk.StringVar(), self.view_state, "selected_behaviour_to_zero"
        )
        self.selected_photometry_line_width = bind_tk_var(
            tk.StringVar(), self.view_state, "selected_photometry_line_width"
        )
        ...
        if self.app_name == "align_photometry_and_behaviour_app":
            ...
            self.view_state.duration_box_placement = str(
                self.settings_manager.duration_box_placement
            )
            self.duration_box_placement.set(self.view_state.duration_box_placement)
            self._bind_settings_manager_var(
                self.selected_photometry_line_width,
                self.settings_manager,
                "selected_photometry_line_width",
            )
            self._bind_settings_manager_var(
                self.box_height_factor, self.settings_manager, "box_height_factor"
            )
            self._bind_settings_manager_var(self.alpha, self.settings_manager, "alpha")
            self._bind_settings_manager_var(
                self.bar_graph_size, self.settings_manager, "bar_graph_size"
            )
            self._bind_settings_manager_var(
                self.onset_line_thickness,
                self.settings_manager,
                "onset_line_thickness",
            )
            self._bind_settings_manager_var(
                self.onset_line_style, self.settings_manager, "onset_line_style"
            )
            self._bind_settings_manager_var(
                self.duration_box_placement,
                self.settings_manager,
                "duration_box_placement",
            )
    ...
    def create_onset_line_style_widget(self):
        ...
        self.onset_line_style_combobox = ttk.Combobox(self.graph_settings_frame, width=7, values=onset_line_styles,
                                                      state='readonly', textvariable=self.onset_line_style)
    ...
    def create_display_options_widgets(self):
        ...
        duration_box_checkbox = tk.Checkbutton(self.graph_settings_frame, text='Display Duration',
                                               font=('Helvetica', 8),
                                               variable=self.display_duration_box_var, bg='snow',
        ...
        instances_box_checkbox = tk.Checkbutton(self.graph_settings_frame, text='Display Number of Instances',
                                                wraplength=120,
                                                font=('Helvetica', 8),
                                                variable=self.num_instances_box_var, bg='snow',
                                                command=self.refresh_graph_display)
    ...
    def create_axis_range_widget(self):
        ...
        self.limit_axis_range_var = bind_tk_var(
            tk.BooleanVar(), self.view_state, "limit_axis_range"
        )
    ...
    def create_zero_x_axis_widget(self):
        ...
        self.zero_x_axis_checkbox_var = bind_tk_var(
            tk.IntVar(), self.view_state, "zero_x_axis_to_behaviour"
        )
    ...
    def save_and_close_photometry_settings(self, popup):
        ...
        if hasattr(self, "horizontal_scale_bar_offset_var"):
            self.settings_manager.selected_horizontal_scale_bar_offset = (
                self.horizontal_scale_bar_offset_var.get()
            )
        if hasattr(self, "vertical_scale_bar_offset_var"):
            self.settings_manager.selected_vertical_scale_bar_offset = (
                self.vertical_scale_bar_offset_var.get()
            )
        if hasattr(self, "scale_bar_color_var"):
            self.settings_manager.selected_scale_bar_color = (
                self.scale_bar_color_var.get()
            )
        if hasattr(self, "scale_bar_width_var"):
            self.settings_manager.selected_scale_bar_width = (
                self.scale_bar_width_var.get()
            )
        if hasattr(self, "scale_bar_size_var"):
            self.settings_manager.selected_scale_bar_size = (
                self.scale_bar_size_var.get()
            )
    ...
    def save_and_close_temperature_settings(self, popup):
        ...
        - popup (tk.Toplevel): The pop-up window to close.
        """
        self.settings_manager.selected_temp_mean_line_width = self.settings_vars['Temperature']['mean_line_width'].get(
        )
        self.settings_manager.selected_temp_line_width = self.settings_manager.selected_temp_mean_line_width
