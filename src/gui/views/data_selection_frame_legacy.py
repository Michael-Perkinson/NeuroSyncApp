...
class DataSelectionFrame(ttk.Frame):
    ...
    def _create_column_title_selector(self, parent_frame):
        ...
        """
        self.column_label = tk.Label(parent_frame, text="Column Title:", font=(
    ...
    def _create_baseline_note_frame(self):
        ...
        baseline_checkbox = ttk.Checkbutton(self.baseline_note_frame, text="Baselined z-score", variable=self.use_baseline_var,
        ...
        baseline_start_label = tk.Label(self.baseline_note_frame, text="Start Time (s):", font=('Helvetica', 10),
                                        fg='black', bg='snow')
        baseline_start_label.grid(row=0, column=1, sticky=tk.W)

        self.baseline_start_entry = tk.Entry(self.baseline_note_frame, width=10, font=('Helvetica', 10), fg='black', bg='snow',
                                             state=tk.DISABLED)
        ...
        baseline_end_label = tk.Label(self.baseline_note_frame, text="End Time (s):", font=('Helvetica', 10),
                                      fg='black', bg='snow')
        baseline_end_label.grid(
            row=0, column=3, padx=(10, 0), sticky=tk.W)

        self.baseline_end_entry = tk.Entry(self.baseline_note_frame, width=10, font=('Helvetica', 10), fg='black', bg='snow',
                                           state=tk.DISABLED)
    ...
    def _create_baseline_save_button(self):
        ...
        self.baseline_save_button = tk.Button(self, text="Save Baseline", font=('Helvetica', 10), bg='lightblue',
            ...
            try:
                ...
                if file_path.endswith('.csv'):
                    with open(file_path, 'r') as file:
                        ...
                        for i, line in enumerate(file, start=1):
                            ...
                            if first_value.replace('.', '', 1).isdigit():
                                dataframe = pd.read_csv(file_path, skiprows=i, header=None)
                    ...
                    if dataframe is None or dataframe.empty:
                        raise ValueError("Could not find a valid data row in the CSV file.")
                    ...
                    for i in range(5):
                        try:
                            ...
                            if str(temp_df.columns[0]).replace('.', '', 1).isdigit():
                                dataframe = pd.read_excel(file_path, skiprows=i, header=None)
                                dataframe.columns = column_titles
                                break
                        except Exception:
                            continue
                    ...
                    if dataframe is None or dataframe.empty:
                        raise ValueError("Could not find a valid data row in the Excel file.")
                else:
